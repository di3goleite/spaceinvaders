 ...
 * @author lucas
 */
public class NaveInimigaCompostaRapidaPequena implements INaveInimiga, AlienListener {
    ...
    public void mover() {
        ...
        timer = new Timer();
        timer.schedule(new NaveInimigaCompostaRapidaPequena.Move(), 0, 1000/10);
    ...
    private class Move extends TimerTask {
        ...
        @Override
        public void run() {
            if ((caixaX >= paredeX - 10 && orientacao == 1)
                    || (caixaX0 <= 1 && orientacao == 0)) {
                for (INaveInimiga alien : aliens) {
                    alien.mover(1);
